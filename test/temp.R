...
# # # 
# # # library(flipMaxDiff)
# # # MaxDiffDesign(number.alternatives = 10, 
...
# # # ###
# # # 
# # # pizze = c("miješana", "povrtna", "pikantna", "4 vrste sira", "s tunom", "ribarska", "4 godišnja doba",
...
# # # shown = 3 # koliko se prikazuje u svakom pitanju?
# # # vers = 3 # koliko varijanti upitnika?
# # # tasks = ceiling((items * 3)/shown/vers) + 1 # koliko pitanja po upitniku?
# # # 
# # # library(choiceDes)
# # # dz = tradeoff.des(items = items, shown = shown, vers = vers, tasks = tasks, fname = NULL, Rd = 500, Rc = 10000, print = TRUE)
# # # cp.scree(dz) # da li se critical stabilizirao?
# # # dz$design
# # # 
# # # 
# # 
# choicemodelr__ = function (data, xcoding, demos, prior, mcmc, constraints, options)
# {
#   callStop = function(message) {
#     stop(message, call. = FALSE)
#   }
#   if (missing(data)) {
#     callStop("data argument required")
#   }
#   if (ncol(data) < 5) {
#     callStop("there are not enough columns in data to specify ID, Set, Alt, X variable(s), and y")
#   }
#   natts = ncol(data) - 4
#   if (missing(xcoding)) {
#     callStop("xcoding argument required")
#   }
#   if (length(xcoding) != natts) {
#     callStop("length of xcoding is not equal to number of attributes")
#   }
#   if (any(xcoding != 0 & xcoding != 1)) {
#     callStop("xcoding must contain only values 0 and 1")
#   }
#   npw = sum(xcoding == 0)
#   ID = unique(data[, 1])
#   nunits = length(ID)
...
#   if (drawdelta) {
#     if (nrow(demos) != nunits) {
#       callStop("number of rows in demos does not equal number of units")
#     }
#     nz = ncol(demos)
...
#     if (wgt < 1 | wgt > 10) {
#       callStop("wgt must be in the range 1 to 10")
#     }
#   }
#   maxsets = max(data[, 2])
...
#   if (all(data[!(data[, 3] == 1), ncol(data)] == 0)) {
#     share = FALSE
#   }
#   else {
#     share = TRUE
...
#       for (i in 1:nunits) {
#         for (j in 1:nsets[i]) {
#           ind = (data[, 1] == ID[i]) & (data[, 2] ==
#                                           j)
#           Xt = rbind(Xt, cbind(data[ind, c(-1, -2, -3,
#                                            -ncol(data)), drop = FALSE], 0), c(rep(0,
#                                                                                   natts), 1))
#         }
#       }
#     }
...
#   }
#   if (none) {
#     effectsmap = rbind(effectsmap, c(rep(FALSE, npar - 1),
#                                      TRUE))
#   }
#   if (missing(prior)) {
#     prior = list()
...
#     if (ncol(mubar) != npar) {
#       callStop("ncol(mubar) does not equal npar")
#     }
#   }
...
#     if (any(dim(Amu) != c(1, 1))) {
#       callStop("Amu must be a 1 x 1 matrix")
#     }
#   }
#   if (is.null(prior$df)) {
#     df = 5
...
#     df = prior$df
#     if (df < 2) {
#       callStop("invalid value of df - must be >= 2")
#     }
#   }
#   nu = npar + df
#   if (is.null(prior$v)) {
...
#     v = prior$v
#     if (v <= 0) {
#       callStop("invalid v - must be >= 0")
#     }
#   }
#   if (is.null(prior$Ad) & drawdelta) {
#     Ad = 0.01 * diag(npar * nz)
#   }
#   else if (drawdelta) {
#     Ad = prior$Ad
#     if (any(dim(Ad) != c(npar * nz, npar * nz))) {
#       callStop("Ad must be of dimensions (npar * nz) x (npar * nz)")
#     }
#   }
#   if (is.null(prior$deltabar) & drawdelta) {
#     deltabar = rep(0, nz * npar)
#   }
#   else if (drawdelta) {
#     deltabar = prior$deltabar
#     if (length(deltabar) != nz * npar) {
#       callStop("deltabar must be of length npar * nz")
#     }
#   }
...
#   V = V * nu * v
#   if (missing(mcmc)) {
#     callStop("mcmc argument required")
#   }
#   if (is.null(mcmc$s)) {
#     s = 0.1
#     adjust.s = TRUE
...
#   }
#   if (is.null(mcmc$R)) {
#     callStop("element R of mcmc required")
#   }
#   R = mcmc$R
#   if (is.null(mcmc$use)) {
#     use = min(0.5 * R, 10000)
...
#     }
#     f.in = scan("restart.txt", nlines = 1, skip = 0,
#                 quiet = TRUE)
#     fR = f.in[1]
#     fUnits = f.in[2]
#     fPar = f.in[3]
...
#     }
#     if (fUnits != nunits) {
#       callStop("number of units in restart file does not match data file")
#     }
#     if (fPar != npar) {
#       callStop("number of parameters in restart file does not match data file")
#     }
#     if (fCons != as.numeric(constrain)) {
#       callStop("restart file and function arguments do not agree on constraints being present")
#     }
#     if (fDem != as.numeric(drawdelta)) {
#       callStop("restart file and function arguments do not agree on demographics being present")
#     }
#     fIndex = 1
#     fCount = 1
#     repeat {
...
#         break
#       }
#       f.in = scan("restart.txt", nlines = ltr[fCount],
#                   skip = fIndex, quiet = TRUE)
#       switch(fCount, lb <- matrix(f.in, ncol = fPar, byrow = TRUE),
...
#   if (save) {
#     if (drawdelta) {
#       deltadraw = matrix(0, nrow = floor(use/keep), ncol = nz *
#                            npar)
#     }
...
#   if (constrain) {
#     if (save) {
#       betadraw.c = array(0, dim = c(nunits, npar, floor(use/keep)))
#     }
#     if (restart) {
#       oldbetas.c = lbc
#     }
#     else {
#       oldbetas.c = matrix(0, nrow = nunits, ncol = npar)
#     }
#   }
...
#       else {
#         newlength = ncol(constraints[[h]]) - 1
#         betat = betas[, (oldlength + 1):(oldlength +
#                                            newlength), drop = FALSE]
...
#                   betat[change, ] = betat[change, ] -
#                     rowMeans(betat[change, , drop = FALSE])
#                   bad = TRUE
#                 }
#               }
...
#           }
#         }
#         betas[, (oldlength + 1):(oldlength + newlength)] = betat[,
#                                                                  1:newlength]
#       }
#       oldlength = oldlength + newlength
#     }
...
#   }
#   on.exit(sink())
#   sink("RLog.txt", append = TRUE, type = "output",
#        split = TRUE)
#   if (restart) {
#     cat("Restarting from previous ", fR, "-iteration model estmation...",
#         sep = "", fill = TRUE)
#     cat("", fill = TRUE)
#   }
#   cat("                    Logit Data                    ",
...
#   cat("-----------------------------------", fill = TRUE)
#   for (i in 1:natts) {
#     cat(sprintf("%-12s   %-12s   %2i", paste("Attribute",
#                                              i), ifelse(xcoding[i] == 0, "Part Worth", "Linear"),
#                 nlevels[i]), fill = TRUE)
#   }
#   cat("", fill = TRUE)
#   cat(npar, " parameters to be estimated", ifelse(none,
#                                                   " (including 'None').", "."), sep = "",
#       fill = TRUE)
#   cat("", fill = TRUE)
#   cat(nunits, "total units.", fill = TRUE)
#   cat("Average of", round(avgalts, 1), "alternatives in each of",
#       round(avgsets, 1), "sets per unit.", fill = TRUE)
#   cat(ifelse(share, sum(ytab[1, ]), sum(nsets)), ifelse(share,
#                                                         " expanded", ""), " tasks in total.",
#       sep = "", fill = TRUE)
#   cat("", fill = TRUE)
...
#   cat("--------------------", fill = TRUE)
#   for (i in 1:maxalts) {
#     cat(sprintf("%4i    %-6i %s", i, ytab[1, i], paste(ytab[2,
#                                                             i], "%", sep = "")), fill = TRUE)
#   }
#   cat("", fill = TRUE)
#   cat("      MCMC Inference for Hierarchical Logit       ",
#       fill = TRUE)
#   cat("==================================================",
#       fill = TRUE)
...
#   }
#   cat("Constraints ", ifelse(constrain, "", "not "),
#       "in effect.", fill = TRUE, sep = "")
#   cat("Draws ", ifelse(save, "are to be ", "not "),
#       "saved.", fill = TRUE, sep = "")
...
#   for (rep in 1:R) {
#     if (drawdelta) {
#       mgout = rGibbs(oldbetas - demos %*% t(matrix(olddelta,
#                                                    ncol = nz)), mubar, Amu, nu, V)
#       oldcomp = mgout
#       olddelta = drawDelta(demos, oldbetas, oldcomp, deltabar,
#                            Ad)
#     }
#     else {
...
#     if (drawdelta) {
#       betabar = t(matrix(olddelta, ncol = nz) %*% t(demos) +
#                     oldcomp$mu)
#     }
#     else {
#       betabar = oldcomp$mu
...
#                                   s, inc.root, betabar, rootpi, info, constraints,
#                                   oldbetas.c)
...
#       PctCert = (mean(oldll) - log(1/avgalts) * avgsets *
#                    wgt)/(-log(1/avgalts) * avgsets * wgt)
#     }
#     else {
...
#     muplot[rep, ] = getEffectsCodedParameters(mutemp, effectsmap,
#                                               xcoding)
#     newplot = FALSE
#     if (rep == 1) {
#       yl = c(-1, 1)
#       matplot(0, 0, type = "l", col = 1:8, lty = 1,
#               ylim = yl, xlim = c(1, R + fR), xlab = "Rep",
#               ylab = "Mu")
#     }
#     if (rep%%100 == 0) {
...
#       }
#       if (newplot) {
#         matplot(xplot[1:rep], muplot[1:rep, ], type = "l",
#                 col = 1:8, lty = 1, ylim = yl, xlim = c(1,
#                                                         R + fR), xlab = "Rep", ylab = "Mu")
#       }
#       else {
#         matplot(xplot[(rep - 100):rep], muplot[(rep -
#                                                   100):rep, ], type = "l", col = 1:8, lty = 1,
#                 add = TRUE)
#       }
#       ctime = proc.time()[3]
#       timetoend = ((ctime - itime)/rep) * (R + 1 - rep)
#       if (rep == 100) {
#         cat("Iteration ", "Acceptance  ",
#             "RLH    ", "Pct. Cert.  ", "Avg. Var.  ",
#             "RMS    ", "Time to End", fill = TRUE)
#       }
#       cat(sprintf("%9.0i  %-5.3f        %-5.3f   %-5.3f        %-5.2f       %-5.2f   %-6s",
#                   rep + fR, acceptr.t/(100 * nunits), RLH.a, PctCert.a,
#                   AvgVar.a, RMS.a, paste(timetoend%/%60, ifelse(round(timetoend%%60) >
#                                                                   9, ":", ":0"), round(timetoend%%60),
#                                          sep = "")), fill = TRUE)
#       fsh()
#       acceptr.t = 0
#     }
...
#           if (constrain) {
#             betadraw.c[, , mkeep] = oldbetas.c
#           }
#           if (drawdelta) {
#             deltadraw[mkeep, ] = olddelta
#           }
...
#   ctime = proc.time()[3]
#   cat("", fill = TRUE)
#   cat("Total Time Elapsed: ", (ctime - itime)%/%60, ifelse(round((ctime -
#                                                                     itime)%%60) > 9, ":", ":0"), round((ctime -
#                                                                                                           itime)%%60), fill = TRUE, sep = "")
#   cat("", fill = TRUE)
#   write.table(cbind(R + fR, nunits, npar, s, as.numeric(constrain),
#                     as.numeric(drawdelta)), "restart.txt", sep = " ",
#               row.names = FALSE, col.names = FALSE)
#   write.table(oldbetas, "restart.txt", sep = " ",
#               row.names = FALSE, col.names = FALSE, append = TRUE)
#   if (constrain) {
#     write.table(oldbetas.c, "restart.txt", sep = " ",
#                 row.names = FALSE, col.names = FALSE, append = TRUE)
#   }
#   if (drawdelta) {
#     write.table(t(olddelta), "restart.txt", sep = " ",
#                 row.names = FALSE, col.names = FALSE, append = TRUE)
#   }
#   betaout = betaout/use
...
#       betanames = c(betanames, paste("A", i, "B",
#                                      j, sep = ""))
#     }
#   }
#   if (none) {
...
#   }
#   cat("Writing estimated unit-level betas to Rbetas.csv in the working directory",
#       fill = TRUE)
#   cat("", fill = TRUE)
#   write.table(betawrite, file = "RBetas.csv", sep = ",",
#               col.names = betanames, row.names = FALSE, qmethod = "double")
#   if (save) {
...
# }
# 
# hb.post = choicemodelr__(data = cmr.mm.bez.cov,
...
# # alt = 1:30
# # nalts = 3
# # for (q in 1:(length(alt)/nalts)) {
# #   paste("q=", q)
# #   i_start = (q-1)*nalts+1
# #   i_end = q*nalts
# #   paste("i_start=", i_start, "i_end=", i_end)
# #   for (i in i_start:i_end) {
# #     print((q-1)*2*nalts+i-(q-1)*nalts)
# #     print((q-1)*2*nalts+i-(q-1)*nalts+nalts)
# #   }
# # }
# # 
# # 
# # 
# 
# #mm = domodelmatrix(cbc.df$alt, designctx$nalternatives, getdfcolumns(cbc.df, cnames = names(designctx$covariates)))
# 
# 
