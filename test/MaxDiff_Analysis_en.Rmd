...
In this document we will present a test research with the MaxDiff method. The research is based on the (assumed) wish of a local pizzeria to find out which new pizza would its buyers prefer to see in its offering.
...
Except the main products (pizzas) that are in our fouces, we will also take into account a possible influence of the "demographic" factors to the answers/results:
...
Out of the set of all `r choose(length(designctx$items), designctx$nalternatives)` products combinations with `r designctx$nalternatives` alternatives (number of alternatives is set as a design parameter), we have chosen  `r nrow(designctx$survey)` representative ones. With the chosen combinations it is possible to calculate (average or individual) value of each product for the respondents.
...
For better estimation of the values of different proudcts, we added a question in which a respondent expresses general positive or negative tendency to a few given products. The products given in this question are chosen related to the previous respondent's choices, so that a span, from the most often chosen as best to the most often chosen as worst pizzas, are given.
In this question products are not compared with each other but all against a so called threshold option, which can mean different things in different contexts, but generally means the point between want/don't want, would buy/wouldn't buy, am interested/am not interested etc. This questions helps us to better level the answers of all the respondents against the threshold option and thus against each other, giving us better and more relevant product values estimates.
...
There are multiple ways to analyze the data with a MaxDiff method. The simplest approach is to count how many times some product was chosen as the best and the worst. Here is our list with the number of times chosen as best/worst:
...
* 'lovačka' and 'losos' both have positive (mean) coefficients (`r as.character(round(sm['lovačka', 'Mean'], 3))`, `r as.character(round(sm['losos', 'Mean'], 3))`) which means that the respondent value both of them more than the basic threshold option, but 'lovačka' is valued more than 'losos' - to put it simply, respondents would buy both of them, but 'lovačka' rather (much more) than 'losos',
...
These results show the main advantages of the MaxDiff analysis compared to simple product "ordering" from the best to the worst which is sometimes (too often :)) used for business analyses. MaxDiff shows us:

* *relations between products* - how much is each more or less valuable to the respondents,
...
Pogledajmo kako izgleda nekoliko redaka iz reach & frequency tablice za naše proizvode (poredanih silazno po reach):
...
Možemo vidjeti da bi kombinaciju `r rownames(combs_r_f)[1]` kupilo najviše ispitanika, `r combs_r_f[1, 1]`, što je `r round(combs_r_f[1, 1]/length(unique(cbc.df$resp.id)), 2)*100`%, s time što bi ukupno bilo `r combs_r_f[1, 2]` (potencijalnih) kupovina.
