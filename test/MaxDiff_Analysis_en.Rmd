...
We can see that the offer `r rownames(combs_r_f)[1]` would reach (at least one product would be bought by) the most respondents, `r combs_r_f[1, 1]` of them, which is `r round(combs_r_f[1, 1]/length(unique(cbc.df$resp.id)), 2)*100`% of the total number of respondents, with a total of `r combs_r_f[1, 2]` (potential) sales.
