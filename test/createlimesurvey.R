...
for (i in 1:length(questionnaires)) {
  ...
  if (designctx$anchors > 0) {
    ...
    sAnchorQuestion1 = enc2utf8("<p>Kad biste imali mogućnost kupovine ovih opcija, da li biste ih kupili ili ne?</p>")
    ...
    for (sqidi in 1:length(limesurveyanchorsubquids)) {
      ...
      limesurveyanchorqids[(limesurveyanchorqids$vers == i), sqidi+1] = 
        paste(iNewID, "X", iGid2, "X", iQid1, limesurveyanchorsubquids[sqidi], sep = "")
    }
