---
title: Themes for Facebook Fans (Case Study)
...
In this document we will present a survey about the themes that our client's (food industry) Facebook fans find the most interesting. The goal was to increase the activity and interactions on the Facebook page by finding the most appealing themes and adapting the content production accordingly.
We used the MaxDiff method in which respondents pick the most/least interesting theme in each question. MaxDiff method makes the survey easy to fill thus leading to better respondents' answers and better/more relevant results.
...
The total number of themes we tested is `r length(designctx$items)`.

From the set of all `r choose(length(designctx$items), designctx$nalternatives)` combinations of themes with `r designctx$nalternatives` alternatives (this many options are in each question), we have chosen `r nrow(designctx$survey)` representative ones. With the chosen combinations it is possible to calculate (average or individual) value of each theme for the respondents.

All combinations were divided into `r designctx$nquestionnaires` questionnaires, each with `r designctx$nquestions` questions, each with `r designctx$nalternatives` options/alternatives.

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("upitnik_2.jpg")
```

We have also asked the respondents a few questions in which they expressed a generally positive or negative attitude (interest) for a few chosen themes. These questions give us a way to level all the respondents' answers (choices) with respect to the "anchor" or "boundary" option representing interested/not interested boundary and thus also with respect to each other. This gives us relevant and usable results.

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("anchoring_survey.jpg")
```

We used the Facebook promotion mechanism to reach the Facebook page fans. Facebook promoted post reached around 31000 people and we got around 800 clicks to our survey (CTR = 2.6%). These 800 clicks resulted in around 200 filled surveys.


## Analysis

The data (respondents choices) were analyzed with the Hierarchical Bayes multinomial logit model. The Hierarchical Bayes model gives us individual estimates of coefficients/values for each respondent.

The data were first adapted for the calculation, and then the coefficients for each researched theme were calculated. We got the following summary (the actual theme names are scrambled):
...
scrambled_names = list()
rnames = rownames(psm)
snames = paste("theme_", 1:nrow(psm), sep = "")
for (i in 1:length(rnames)) {
  scrambled_names[[rnames[i]]] = snames[i]
}
rownames(psm) = snames
psm
...
In the summary shown are some figures that are not so easy to interpret directly. But to clarify a little bit, here are some explanations that follow directly from the summary:

* `r scrambled_names[['slatke.kiflice.i.peciva.s.punjenjem..pekmez..Nutella..']]` i `r scrambled_names[['slane.kiflice.i.peciva.s.punjenjem..šunka..sir..']]` both have positive (average/mean) coefficients (`r as.character(round(sm['slatke.kiflice.i.peciva.s.punjenjem..pekmez..Nutella..', 'Mean'], 3))`, `r as.character(round(sm['slane.kiflice.i.peciva.s.punjenjem..šunka..sir..', 'Mean'], 3))`) which means that the respondents are (on average) interested in both themes, but for `r scrambled_names[['slane.kiflice.i.peciva.s.punjenjem..šunka..sir..']]` more than for  `r scrambled_names[['slatke.kiflice.i.peciva.s.punjenjem..pekmez..Nutella..']]`,

* 1st and 3rd quantiles mark the range in which (somewhat simplified) is each coefficient ("value") with 75% probability.

We have normalized and scaled all the coefficients so that the anchor boundary (interested/not interested) equals 100. This way it is easier to interpret the theme values and their relations:
...
ggplot(data = data.frame(product = paste(unlist(scrambled_names), " (", round(perc.zc.sm, digits = 2), ")", sep = ""), value = perc.zc.sm), aes(x = product, y = value)) + geom_col(fill = "blue") + geom_hline(aes(yintercept = 100), linetype = "dashed", color = "red") +  labs(y = "not interested.......interested", color = "red") + theme(axis.title.x = element_text(hjust = .33), axis.title.y = element_blank()) + coord_flip()
...
moreless = if (rationmostcomp > 1) "bigger" else "smaller"

```

Now we can see that e.g. the most interesting subject is **`r scrambled_names[[mostvpn]]`** and that its "popularity" is **`r rationmostcomp` times `r moreless`** than the "popularity" of `r scrambled_names[[compvpn]]`.

These results also show the main advantages of the MaxDiff analysis compared to a simple product "ordering" from the best to the worst which is still used too often for business analyses. MaxDiff shows us:

* *relations between products* - how much is each product more or less valuable for the respondents,
* product positions *with respect to the (key!) point marking the positive or negative attitude* - would buy or not, am interested or not...

This way we get a complete picture of all the products from the respondents' perspective and can then make quality business decisions.


## Share Prediction

With all the themes' coefficients calculated, we can combine them into (arbitrary) groups and predict how interesting would these themes be for the respondents if only the themes in a group were compared.
As an example, we will pick a combination of a few randomly chosen themes and calculate their predicted shares in "popularity":
...
pshares = t(shares)
pshares["opcije", ] = sapply(make.names(shares$opcije), function(n) scrambled_names[[n]])
pshares
...
In the previous table we can see how would the respondents choose between the `r nrow(ffd_df)` chosen themes. Each theme gets a certain share (percentage) in the total "popularity", according to the estimated coefficients values.


The most popular theme (among the listed themes, not in general) is `r scrambled_names[[make.names(as.matrix(mostshare[, -ncol(mostshare)]))]]` with the predicted "popularity" share of `r max(shares[["share %"]])`%.



## Checking the model on the existing data

Using the predicted shares/sales probabilities for different theme combinations, we can test how would our model predict the choices for (some of) the combinations given in the survey:
...
paste("Total correct predictions:", round(100*total_correct/(2*nrow(holdout_true_predicted)), digits = 2), "%", sep = "")

```

The overall prediction accuracy makes us confident that our model works well.


## Conclusion

We can conclude that the most popular themes for the client's Facebook fans are:
...
rownames(zanimaju_me) = sapply(rownames(zanimaju_me), function(n) scrambled_names[[n]])
zanimaju_me

```

According to the given results we adapted communication and content production. This resulted in a 50% activity increase on the Facebook page.

```{r, echo=FALSE, fig.align='center'}
knitr::include_graphics("activity_increase.jpg")
```
