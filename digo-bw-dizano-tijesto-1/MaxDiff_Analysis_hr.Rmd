...
cc = cov2cor(crossprod(Reduce('+', lapply(1:length(hb.post$compdraw), function(i) {
  hb.post$compdraw[[i]]$rooti
}))/length(hb.post$compdraw)))
rownames(cc) = colnames(cc) = colnames(cbc.mm.clear)
...
ggplot(data = correlation_t, aes(x = substr(correlation_t$names, 1, 18), y = correlation)) + geom_col(color = "red", fill = "pink", width = .03) +
  geom_hline(aes(yintercept = 0), color = "red") + xlab("") + ggtitle(paste("Correlations between", paste(substr(corvar, 1, 30), "..", sep = ""), "and other values"))
...
#   cfs_s_diffs <<- unique(c(cfs_s_diffs, names(which(abs(stan.mc.lca.betas.mean[, s] - stan.mc.lca.betas.mean[, (s+1)]) > 0.5))))  
# })
### STAN MC LCA <<<
```
...
if (!is.null(simcoefs) & !is.null(simcoefs$beta) & !is.null(simcoefs$seg.df)) {
  ...
  for (i in 1:nrow(all_seg_perms)) {
    km1cluster_permuted = rep(0, length(km1$cluster))
    for (c in 1:designctx$rsegments) {
      km1cluster_permuted[km1$cluster == c] = all_seg_perms[i, c]
    }
    seg_perm_score[i] = sum(true_segments$seg == km1cluster_permuted)
  }
  ...
  table(true_segments$seg == km1$cluster)
  paste("Correct segmentation: ", round(sum(true_segments$seg == km1$cluster)/length(true_segments$seg), 2)*100, "%", sep = "")
